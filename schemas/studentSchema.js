...
studSchema.statics.getAllUserQueryLimit = async (offsetN, string, opt1, opt2) => {
    ...
    if (Number(opt1) === 10 && opt2 === 'All')
        var user = await Stud.find({ passed: false, name: { '$regex': `${string}`, '$options': 'i' } }).sort({ roll: 1 }).skip(skipInNumber).limit(25);
...
studSchema.statics.getAllUserLimit = async (offsetN, opt1, opt2) => {
    ...
    var user;
    if (Number(opt1) === 10 && opt2 === 'All')
        var user = await Stud.find({ passed: false }).sort({ roll: 1 }).skip(skipInNumber).limit(25);
    else if (Number(opt1) !== 10 && opt2 === 'All')
